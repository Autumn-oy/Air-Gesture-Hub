...
# ---- 3b. ★ 冷静期从"触发那一瞬间"起算，定稿 1000ms（v2.3.2 由 1100 改） ----
# 触发后约 0.8s 就想反向再扫一次（姿势也已停稳）：仍在 1000ms 窗口内，必须被忽略。
add("cooldown_blocks_early_second_sweep", {},
    ...
    + hold(0.5, 0.60, 40),
    "定稿 1000ms：触发后约 0.8s 的反向扫动必须被完全忽略 -> 只触发一次")
...
add("cooldown_allows_sweep_after_expiry", {},
    ...
    "过了 1000ms 冷静期并重新停稳后，第二次扫动必须触发（up, down）")
